...
- **Level Manager:** Handles seed-based procedural generation of game levels — placing authored room templates, connecting them with corridors, resolving doors, placing occlusion, baking navigation, and spawning the player and enemies. Connectivity is doorway-driven: explicit doorways are always connected while inferred open edges are optional, and a spanning tree guarantees every room is reachable. See `docs/level-design.md` for the full pipeline.
- **Room Authoring:** Room scenes combine physical layout (`GridMap` floor/wall/decoration data) with editor-visible semantic markers for authored intent such as doorway connection points. GridMaps remain the source of room shape and passability, while markers describe gameplay meaning that generators and runtime systems can consume.
- **Map Visibility (Fog of War):** A runtime occlusion `GridMap` of flat black caps hides unreachable void and undiscovered areas. Rooms are revealed on entry (`RoomManager` → `FogOfWar`), cascading through door-free connectors but stopping at closed doors; opening a door reveals beyond it. Newly revealed areas fade in smoothly via a short-lived overlay rather than popping, while restoring a save reveals instantly. Closed doors show an occlusion-aware x-ray silhouette so they remain visible at any camera angle, and reveal state is persisted per dungeon depth in the save.
- **Presentation Fades:** A reusable `FadeOutComponent` eases 3D content out via material/modulate alpha (working on every renderer) instead of disappearing in a single frame. It backs dying enemies (`DeathComponent`) and is available for props or other despawning actors; the fog reveal uses the same alpha approach for its overlay.
...
- **Enemy AI:** Controls the behavior of non-player characters. Enemies are scene-composed actors with reusable behavior components and resource-driven behavior profiles for monster-specific tuning such as detection, roaming, attack ranges, and action timing. Each enemy's brain is a small state machine (idle, patrol, chase, search, flee) sitting above dedicated perception (sight/hearing) and navigation (navmesh path-following) components, with a timed-action layer gating animations and attacks. More specialized monsters should prefer new scenes/resources before adding global systems.
...
- **Stats & Itemization:** Player stats are a composable layer rather than fixed fields. Primary attributes (Strength, Dexterity, Vitality, Intelligence) plus base secondary stats resolve as `base + attribute derivation + active modifiers` (additive then multiplicative); a `StatProfile` resource holds the attribute-to-stat derivation rules and is the seam classes/races plug into. Buffs and equipment contribute `StatModifier`s registered under their source object, so any effect reverses exactly without drift. Equipment that drops rolls a depth-scaled rarity and randomized affixes (`Affix`/`AffixPool` produce per-instance `RolledAffix`es) using the per-run seeded loot RNG; a rolled item is a duplicate of the shared definition and is persisted with its rarity and affixes. Every equipable also carries a `Tier` (the base item's ladder rung, independent of rolled rarity), and loot table entries carry their own depth eligibility so one authored table serves every depth. This is distinct from the type-level identification disguise system. See `docs/stats-and-itemization.md`.
- **Item Identification:** Potions, scrolls, and jewelry (rings/amulets) hide their true identity behind a per-run disguise (a random appearance assigned to each item *type*, not each instance) until discovered by use, wearing, or a scroll of identify — drawing on the original *Rogue*. `IIdentifiable` is the shared contract across the two identity-bearing branches (`IdentifiableItem` for consumables, `IdentifiableEquipableItem` for jewelry), since C# single inheritance keeps them from sharing a base class. Scroll effects that aren't stat buffs (identify, enchant, teleport, summon) are authored `ScrollEffect` strategies rather than bespoke item subclasses; effects needing a chosen item route through an inventory targeting mode. See `docs/item-identification-system.md`.
- **Character Classes:** Playable classes are designer-authored `CharacterClass` resources (visual model, attribute block, class `StatProfile`, starting gear, pre-identified item types) collected in an ordered `CharacterCatalog`. The class is chosen in a menu carousel at new-game start, persisted per save slot by id, resolved by `GameSession`, and applied by the player on spawn — including swapping the shared-rig KayKit model at runtime. Classes supply the `StatProfile` seam described above. See `docs/character-classes.md`.
